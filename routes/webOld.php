...
use App\Http\Controllers\TransportWizardController;
...
Route::middleware(['auth'])->group(function () {
    Route::middleware('role:ceo|it_manager')->group(function () {
        Route::get('/access-matrix', [AccessMatrixController::class, 'index'])->name('access.matrix.index');
        Route::post('/access-matrix/roles', [AccessMatrixController::class, 'updateRoles'])->name('access.matrix.updateRoles');
        Route::post('/access-matrix/users', [AccessMatrixController::class, 'updateUserOverrides'])->name('access.matrix.updateUsers');
    });
});


Route::middleware(['auth', 'route.permission', 'role:ceo|it_manager'])->group(function () {
    Route::get('/access-matrix', [AccessMatrixController::class, 'index'])->name('access.matrix.index');
    Route::post('/access-matrix/roles', [AccessMatrixController::class, 'updateRoles'])->name('access.matrix.updateRoles');
    Route::post('/access-matrix/users', [AccessMatrixController::class, 'updateUserOverrides'])->name('access.matrix.updateUsers');

    // Ajax: دریافت وضعیت permissionهای یک کاربر
    Route::get('/access-matrix/user/{user}', [AccessMatrixController::class, 'userPermissions'])
        ->name('access.matrix.userPermissions');
});
...
Route::prefix('pre-invoices')->name('pre_invoices.')->group(function () {
    ...
    Route::get('{preInvoice}', [PreInvoiceController::class, 'show'])->name('show');
    ...
    // Route::put('{preInvoice}/sales-manager-decision', [PreInvoiceController::class, 'salesManagerDecision'])->name('sales-manager-decision');
    ...
    // web.php
    Route::get('{preInvoice}/transports/{transport}/buy', [TransportPurchaseController::class, 'edit'])->name('transports.buy');

});


Route::prefix('pre-invoices/{preInvoice}')->name('pre_invoices.')->group(function () {
    ...
    Route::get('transports/{transport}', [TransportWizardController::class, 'show'])->name('transports.wizard.show');

    // آپدیت هر مرحله جداگانه
    Route::post('transports/{transport}/step/sales', [TransportWizardController::class, 'updateSales'])->name('transports.wizard.update.sales');

    Route::post('transports/{transport}/step/purchase', [TransportWizardController::class, 'updatePurchase'])->name('transports.wizard.update.purchase');

    Route::post('transports/{transport}/step/logistics-manager', [TransportWizardController::class, 'updateLogisticsManager'])->name('transports.wizard.update.logistics_manager');

    Route::post('transports/{transport}/step/logistics-expert', [TransportWizardController::class, 'updateLogisticsExpert'])->name('transports.wizard.update.logistics_expert');

    Route::post('transports/{transport}/step/accounting', [TransportWizardController::class, 'updateAccounting'])->name('transports.wizard.update.accounting');

    Route::post('transports/{transport}/step/sales-manager', [TransportWizardController::class, 'updateSalesManager'])->name('transports.wizard.update.sales_manager');

    Route::post('transports/{transport}/step/logistics-close', [TransportWizardController::class, 'updateLogisticsClose'])->name('transports.wizard.update.logistics_close');
    
});

// ✅ در انتهای فایل web.php اضافه کنید
// Route::get('/api/products/by-category/{category_id}', function($category_id) {
//     $products = \App\Models\Product::where('category_id', $category_id)->orderBy('name')->get(['id', 'name']);
...
Route::get('/api/products/by-category/{category_id}', function($category_id) {
    $category = ProductCategory::with('children')->findOrFail($category_id);

    // اگر فقط یک سطح child داری:
    // $categoryIds = $category->selfAndChildrenIds();

    // اگر متد recursive ساختی:
...
Route::prefix('transports/{transport}')->name('transports.')->group(function () {

    Route::get('edit', [TransportController::class, 'edit'])->name('edit');

    Route::put('', [TransportController::class, 'update'])->name('update');

    Route::post('vehicles/{vehicle}/accounting-approve', [TransportWizardController::class, 'approveVehicleFreight'])->name('vehicles.accounting-approve');

    Route::post('vehicles/{vehicle}/accounting-reject', [TransportWizardController::class, 'rejectVehicleFreight'])->name('vehicles.accounting-reject');

    Route::post('vehicles/{vehicle}/settle', [TransportWizardController::class, 'settleVehicleFreight'])->name('vehicles.settle');

    Route::post('settle-all', [TransportWizardController::class, 'settleAllTransport'])->name('settle-all');

    // آپدیت وضعیت حسابداری یک وسیله (ماشین / واگن)
    Route::put('vehicles/{vehicle}/accounting', [\App\Http\Controllers\TransportAccountingController::class, 'updateVehicle'])->name('accounting.updateVehicle');

    // تسویه کامل همه‌ی وسایل این فرم حمل
    Route::put('accounting/settle-all', [\App\Http\Controllers\TransportAccountingController::class, 'settleAll'])->name('accounting.settleAll');

    // routes/web.php
    Route::put('sales-manager-approve', [\App\Http\Controllers\TransportSalesController::class, 'approve'])->name('sales-manager.approve');

    // routes/web.php
    Route::put('close', [\App\Http\Controllers\TransportClosingController::class, 'close'])->name('close');



    // routes/web.php
    // Route::get('sales', [TransportSalesController::class, 'show'])->name('transports.sales.show');

    // Route::put('sales', [TransportSalesController::class, 'approve'])->name('transports.sales.approve');
