                ...
                <div class="text-center">
                    <div class="dropdown float-end">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <a class="dropdown-item" href="{{ route('sources.edit', $source->id) }}">ویرایش</a>
                    ...
                    {{-- اطلاعات شرکت منبع اگر وجود داشت --}}
                    @if($source->company)
                        <h5 class="mt-3 mb-1">{{ $source->company->name }}</h5>
                        <p class="text-muted">منبع حقوقی</p>
                        <div class="mb-2 mt-2">
                        ...
                        <div class="text-start mt-3">
                            ...
                            <ul class="list-unstyled mb-3">
                                <li><strong>نام شرکت:</strong> {{ $source->company->name }}</li>
                                <li><strong>کد اقتصادی:</strong> {{ $source->company->economic_code }}</li>
                                <li><strong>شماره ثبت:</strong> {{ $source->company->registration_number }}</li>
                                <li><strong>ایمیل:</strong> {{ $source->company->email }}</li>
                            </ul>
                        </div>

                        @if($source->company->addresses->count())
                            <hr>
                            <div class="text-start mt-2">
                                <h6>آدرس‌های شرکت:</h6>
                                @foreach($source->company->addresses as $address)
                                    @php
                    ...
                    @if($source->first_name || $source->last_name)
                        <h5 class="mt-3 mb-1">{{ $source->first_name }} {{ $source->last_name }}</h5>
                        <p class="text-muted">منبع حقیقی</p>
                        <div class="text-start mt-3">
                            <h6>اطلاعات شخصی</h6>
                            <ul class="list-unstyled mb-3">
                                @if($source->passport_number)
                                    <li><strong>شماره پاسپورت:</strong> {{ $source->passport_number }}</li>
                                @endif
                                @if($source->national_code)
                                    <li><strong>کد ملی:</strong> {{ $source->national_code }}</li>
                                @endif
                                @if($source->birthdate)
                                    <li><strong>تاریخ تولد:</strong> {{ $source->birthdate }}</li>
                                @endif
                                <li><strong>ایمیل:</strong> {{ $source->email }}</li>
                            </ul>
                        </div>
                    @endif
