...
@section('content')
<div class="card">
  <div class="card-header">لیست منابع</div>
  <div class="card-body">

    <!-- فرم جستجو و فیلتر -->
    <form method="GET" action="{{ route('sources.index') }}" class="mb-4">
      <div class="row">
      ...
      <div class="row">
          ...
          <div class="col-md-3 mb-2">
              <select name="province_id" class="form-control" id="filter-province">
                  ...
                  {{-- با Ajax پر می‌شود، مشابه customers --}}
              </select>
          </div>
      ...
      <div class="row">
          ...
          <div class="col-md-2 mb-2">
              <a href="{{ route('sources.index') }}" class="btn btn-secondary w-100">پاک‌کردن</a>
          </div>
          <div class="col-md-3 mb-2">
              <a href="{{ route('sources.create') }}" class="btn btn-success w-100">منبع جدید</a>
          </div>
      </div>
    </form>
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        <tr>
          <td>{{ $source->id }}</td>
          <td>{{ $source->first_name }} {{ $source->last_name }}</td>
          <td>{{ $source->type }}</td>
          <td>{{ $source->company ? $source->company->name : '-' }}</td>
          <td>
            @php
                $addr = $source->addresses->first();
            @endphp
          ...
          <td>
            <a href="{{ route('sources.show', $source) }}" class="btn btn-info btn-sm">مشاهده</a>
            <a href="{{ route('sources.edit', $source) }}" class="btn btn-warning btn-sm">ویرایش</a>
            <form method="POST" action="{{ route('sources.destroy', $source) }}" style="display:inline;">
              @csrf @method('DELETE')
