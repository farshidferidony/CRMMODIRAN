                    ...
                    <select name="country_id" class="form-control">
                        ...
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}"
                                @if(request('country_id') == $country->id) selected @endif>
            ...
            <div class="row">
                <div class="col-md-3 mb-2">
                    <select name="sort_by" class="form-control">
                        ...
                        {{-- مرتب‌سازی بر اساس نام روی Customer نیست، فعلاً غیرفعال --}}
                        {{-- <option value="first_name" @if(request('sort_by')=='first_name') selected @endif>نام</option> --}}
                        <option value="updated_at" @if(request('sort_by')=='updated_at') selected @endif>
                            آخرین ویرایش
                        </option>
                    </select>
                </div>
        ...
        <table class="table table-bordered">
            ...
            @foreach($customers as $customer)
                @php
                    ...
                    // آدرس و کشور
                    $addr = null;
                    if ($person && $person->addresses->isNotEmpty()) {
                        $addr = $person->addresses->first();
                    } elseif ($company && $company->addresses->isNotEmpty()) {
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ route('customers.destroy', $customer) }}"
                            ...
                            <button type="submit" class="btn btn-danger btn-sm"
                                    onclick="return confirm('حذف این مشتری؟')">
                                حذف
                            </button>
                        </form>
                    </td>
